...
def needle_msg_to_frame(msg):
    ...
    frame = Frame(
        ...
        Vector(
            pose_info.position.x / SimToSI.linear_factor,
...
if __name__ == "__main__":
    ...
    for topic, msg, t in bag.read_messages(topics=topics[11]):
        ...
        if prev_time is not None:
            ...
            w = np.arccos(2 * np.dot(cur_ori, prev_ori) ** 2 - 1) / (
                cur_time - prev_time
            )
            w = w * 180 / np.pi
            needle_v.append(v)
    ...
    for topic, msg, t in bag.read_messages(topics=topics[14]):
        ...
        psm1_pos_temp = [
            msg.joint_positions[0],
            msg.joint_positions[1],
            msg.joint_positions[2] / 10.0,
    ...
    if save_action:
        ...
        save_list_file = os.path.join(
            save_folder, f"test_needle.json"
        )  # json file name
        data = {
