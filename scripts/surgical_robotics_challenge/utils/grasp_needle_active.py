...
def gripper_to_yaw(T: Frame) -> Frame:
    L_yaw2ctrlpnt = 0.0
    offset_gripper = Frame(
        Rotation.RPY(0, 0, 0), L_yaw2ctrlpnt * Vector(0.0, 0.0, -1.0)
    )
...
def yaw_to_gripper(T: Frame) -> Frame:
    L_yaw2ctrlpnt = 0.0
    offset_gripper = Frame(Rotation.RPY(0, 0, 0), L_yaw2ctrlpnt * Vector(0.0, 0.0, 1.0))
...
if __name__ == "__main__":
    ...
    time.sleep(0.2)
    psm1 = PSM(
        simulation_manager, "psm1", add_joint_errors=False, tool_id=tool_id_simple
    )
    time.sleep(0.2)
    psm2 = PSM(
        simulation_manager, "psm2", add_joint_errors=False, tool_id=tool_id_simple
    )
    time.sleep(0.2)
    ...
    ## offset of PSM
    offset_psm1 = Frame(
        Rotation.RPY(-np.pi / 2.0, np.pi * 1 / 3, 0.0),
    ...
    T_offset_w = Frame(Rotation.RPY(np.pi, 0.0, 0), Vector(0.0, 0.0, 0.0))
